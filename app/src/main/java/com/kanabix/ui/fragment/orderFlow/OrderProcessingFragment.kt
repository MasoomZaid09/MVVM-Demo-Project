...
import com.kanabix.databinding.FragmentOrderProcessingBinding
...
@AndroidEntryPoint
class OrderProcessingFragment() : Fragment(), OrderManagementClick , sessionExpiredListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.deliveryPartnerOrderListApi(token, "ACCEPTED", "")
    }
    ...
    override fun onCreateView(
        ...
        _binding = FragmentOrderProcessingBinding.inflate(layoutInflater, container, false)
                    ...
                    when (response) {
                        ...
                        is Resource.Success -> {
                            ...
                            if (response.data?.responseCode == 200) {
                                ...
                                fragmentManager?.let {
                                    LogOutDialog(this@OrderProcessingFragment).show(it, "MyCustomFragment")
                                }
                            }
                        }
                        ...
                        is Resource.Error -> {
                            ...
                            data.clear()
                            setAdapter(data)
                            binding.processingRecycler.adapter?.notifyDataSetChanged()
...
//                            }
    ...
    private fun ObserveSearchResponse() {
        sharedSearchViewModel._SearchOrderList.observe(viewLifecycleOwner, Observer { searchText ->
            viewModel.deliveryPartnerOrderListApi(token, "ACCEPTED", searchText)
        })
    }


    private fun setAdapter(data: ArrayList<DeliveryOrderListDoc>) {
        binding.processingRecycler.adapter = OrderPendingAdapter(
            requireContext(),
            data,
            this
        )
