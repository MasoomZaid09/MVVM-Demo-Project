...
import com.kanabix.databinding.FragmentOrderCancelledBinding
...
@AndroidEntryPoint
class OrderCancelledFragment() : Fragment(), OrderManagementClick,sessionExpiredListener{

    private var _binding: FragmentOrderCancelledBinding? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.deliveryPartnerOrderListApi(token, "DECLINED", "")
    }
    ...
    override fun onCreateView(
        ...
        _binding = FragmentOrderCancelledBinding.inflate(layoutInflater, container, false)
                    ...
                    when (response) {
                        ...
                        is Resource.Success -> {
                            ...
                            if (response.data?.responseCode == 200) {
                                ...
                                fragmentManager?.let {
                                    LogOutDialog(this@OrderCancelledFragment).show(it, "MyCustomFragment")
                                }
                            }
                        }
                        is Resource.Error -> {
                            ...
                            data.clear()
                            setAdapter(data)
                            binding.cancelledRecycler.adapter?.notifyDataSetChanged()
                        }
    ...
    private fun ObserveSearchResponse() {
        sharedSearchViewModel._SearchOrderList.observe(viewLifecycleOwner, Observer { searchText ->
            viewModel.deliveryPartnerOrderListApi(token, "DECLINED", searchText)
        })
    }


    private fun setAdapter(data: ArrayList<DeliveryOrderListDoc>) {
        binding.cancelledRecycler.adapter = OrderPendingAdapter(requireContext(), data, this)
    }
